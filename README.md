...
- [Перебор и условия](#задание-1-сложность-тестирование-особые-случаи---10-из-10)
- [Линейный поиск](#задание-2-линейный-поиск---10-из-10)
- [Множества](#задание-3-множества---10-из-10)
- [Словари и сортировка подсчетом](#задание-4-словари-и-сортировка-подсчетом---10-из-10)
- [Префиксные суммы и два указателя](#задание-5-префиксные-суммы-и-два-указателя---10-из-10)
- [Бинарный поиск](#задание-6-бинарный-поиск---11-из-11)
- [Сортировка событий](#задание-7-сортировка-событий---1-из-10)
...
## Задание 6. Бинарный поиск - 11 из 11.
...
**Медиана двух последовательностей.** Определить для каждой пары последовательностей левую медиану объединения. Перебор всех пар последовательностей (не повторяющихся - сдвиг); левая граница медианы - минимальное значение объединения последовательностей; правая граница медианы - максимальное значение объединения последовательностей; бинарный поиск по ответу; перебор значения медианы; определение количества элементов объединенных последовательностей, которые больше и меньше числа (меняется бинарный поиск), проверка краевых условий; проверка определили ли медиану; сдвиг границы.     
Сложность = O(N^2 * logK * logL).   
</br>

**Медиана двух сгенерированных последовательностей.** Определить для каждой пары последовательностей левую медиану объединения. Генерация последовательностей (можно при переборе генерировать 2); перебор всех пар последовательностей (не повторяющихся - сдвиг); левая граница медианы - минимальное значение объединения последовательностей; правая граница медианы - максимальное значение объединения последовательностей; бинарный поиск по ответу; перебор значения медианы; определение количества элементов объединенных последовательностей, которые больше и меньше числа (бинарным поиском, меняется условие), проверка краевых условий; проверка определили ли медиану; сдвиг границы.     
...
**Варианты контрольной.** Распределение вариантов, чтобы не списывали. Сортировка парт по координатам, сдвиг окна для списывания, определения максимальной длины окна, формирование ответа в порядке ввода координат.  
Сложность = O(N * log N).  
</br>

**Количество отрезков в точке.** Подсчет на скольки отрезках находится точка. Формирование событий, сортировка событий, подсчет количества отрезков на текущий момент, формирование ответа в порядке ввода точек.  
